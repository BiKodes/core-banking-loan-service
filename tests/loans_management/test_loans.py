...
class LoanLifecyclePostingTests(TestCase):
    ...
    def setUp(self):
        ...
        self.loan_principal = Account.objects.create(
            code='1210', name='Loan Principal', account_type='ASSET'
        )
        self.fee_receivable = Account.objects.create(
            code='1220', name='Fee Receivable', account_type='ASSET'
        )
        self.interest_income = Account.objects.create(
            code='4100', name='Interest Income', account_type='INCOME'
        )
        self.fee_income = Account.objects.create(
            code='4210', name='Fee Income', account_type='INCOME'
        )
        self.bad_debt = Account.objects.create(
            code='5100', name='Bad Debt Expense', account_type='EXPENSE'
        )
    ...
    def test_disbursement_posts_entries(self):
        ...
        principal_entry = JournalEntry.objects.filter(
            idempotency_key__startswith='loan-disbursement'
        ).first()
    ...
    def test_repayment_posts_entries_and_reduces_outstanding(self):
        serializer = LoanDisbursementSerializer(
            data={'amount': '10000.00'}, context={'loan': self.loan}
        )
        ...
        self.assertGreaterEqual(
            JournalEntry.objects.filter(
                idempotency_key__startswith='loan-repayment'
            ).count(),
            1,
        )
    ...
    def test_writeoff_posts_entries(self):
        serializer = LoanDisbursementSerializer(
            data={'amount': '5000.00'}, context={'loan': self.loan}
        )
        serializer.is_valid(raise_exception=True)
        serializer.save()
