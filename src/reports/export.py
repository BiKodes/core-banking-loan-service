...
def set_header_row(worksheet, row_num: int, headers: List[str], col_widths: Optional[Dict[int, float]] = None):
...
def add_title_section(worksheet, title: str, subtitle: Optional[str] = None, start_row: int = 1) -> int:
    ...
    current_row += 1
    worksheet.cell(row=current_row, column=1, value=f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    return current_row + 2


def add_total_row(worksheet, row_num: int, label: str, value: float, label_col: int = 1, value_col: int = 4):
...
def export_account_balance_report(account_data: Dict[str, Any]) -> Workbook:
    ...
    balance_cell = ws.cell(row=current_row, column=4, value=float(account_data['balance']))
    balance_cell.number_format = '#,##0.00'
    balance_cell.alignment = Alignment(horizontal='right')

    current_row += 2

    add_total_row(ws, current_row, 'Total Debits:', float(account_data['debit_total']), value_col=2)
    current_row += 1
    add_total_row(ws, current_row, 'Total Credits:', float(account_data['credit_total']), value_col=2)

    return wb

def export_transaction_history_report(transactions: List[Dict], account_name: str) -> Workbook:
    ...
    headers = ['Date', 'Trans ID', 'Description', 'Entry Type', 'Debit', 'Credit', 'Balance', 'Status']
    col_widths = {1: 18, 2: 12, 3: 25, 4: 10, 5: 12, 6: 12, 7: 15, 8: 10}
    set_header_row(ws, current_row, headers, col_widths)
    current_row += 1

    for transaction in transactions:
        ws.cell(row=current_row, column=1, value=transaction['date'].strftime('%Y-%m-%d %H:%M:%S'))
        ...
        debit_cell = ws.cell(row=current_row, column=5, value=float(transaction['debit']))
        debit_cell.number_format = '#,##0.00'
        debit_cell.alignment = Alignment(horizontal='right')
        
        credit_cell = ws.cell(row=current_row, column=6, value=float(transaction['credit']))
        credit_cell.number_format = '#,##0.00'
        credit_cell.alignment = Alignment(horizontal='right')
        
        balance_cell = ws.cell(row=current_row, column=7, value=float(transaction['running_balance']))
        balance_cell.number_format = '#,##0.00'
        balance_cell.alignment = Alignment(horizontal='right')
...
def export_trial_balance_report(report_data: Dict[str, Any]) -> Workbook:
    ...
    currency_text = f" - {report_data['currency']}" if report_data['currency'] else ""
    
    current_row = add_title_section(
        ws,
        "TRIAL BALANCE REPORT",
        f"As of {as_of_date}{currency_text}"
    )
    ...
    for line in report_data['lines']:
        ...
        debit_cell = ws.cell(row=current_row, column=3, value=float(line['debit_balance']))
        debit_cell.number_format = '#,##0.00'
        debit_cell.alignment = Alignment(horizontal='right')
        debit_cell.border = BORDER
        
        credit_cell = ws.cell(row=current_row, column=4, value=float(line['credit_balance']))
        credit_cell.number_format = '#,##0.00'
        credit_cell.alignment = Alignment(horizontal='right')
    ...
    ws.cell(row=current_row, column=3, value=float(report_data['total_debits'])).number_format = '#,##0.00'
    ws.cell(row=current_row, column=4, value=float(report_data['total_credits'])).number_format = '#,##0.00'

    current_row += 2
    balance_text = "✓ BALANCED" if report_data['is_balanced'] else "✗ NOT BALANCED"
    balance_cell = ws.cell(row=current_row, column=1, value=balance_text)
    balance_cell.font = Font(bold=True, color="008000" if report_data['is_balanced'] else "FF0000")
...
def export_balance_sheet_report(report_data: Dict[str, Any]) -> Workbook:
    ...
    ws.cell(row=current_row, column=1, value='ASSETS').font = Font(size=12, bold=True, color='FFFFFF')
    ...
    for account in report_data['assets']['accounts']:
        ...
        balance_cell = ws.cell(row=current_row, column=2, value=float(account['balance']))
        balance_cell.number_format = '#,##0.00'
        balance_cell.alignment = Alignment(horizontal='right')
        current_row += 1

    add_total_row(ws, current_row, 'Total Assets', float(report_data['total_assets']), value_col=2)
    current_row += 2

    ws.cell(row=current_row, column=1, value='LIABILITIES').font = Font(size=12, bold=True, color='FFFFFF')
    ws.cell(row=current_row, column=1).fill = PatternFill(start_color='4472C4', end_color='4472C4', fill_type='solid')
    current_row += 1

    total_liabilities = float(report_data['total_liabilities_and_equity']) - float(report_data['equity']['total'])
    ...
    add_total_row(ws, current_row, 'Total Liabilities', total_liabilities, value_col=2)
    current_row += 2

    ws.cell(row=current_row, column=1, value='EQUITY').font = Font(size=12, bold=True, color='FFFFFF')
    ws.cell(row=current_row, column=1).fill = PatternFill(start_color='4472C4', end_color='4472C4', fill_type='solid')
    current_row += 1
    ...
    add_total_row(ws, current_row, 'Total Equity', float(report_data['equity']['total']), value_col=2)
    current_row += 2
...
def export_loan_aging_report(report_data: Dict[str, Any]) -> Workbook:
    ...
    current_row = add_title_section(
        ws,
        "LOAN AGING REPORT - SUMMARY",
        f"As of {as_of_date}{currency_text}"
    )

    headers = ['Aging Bucket', 'Days Range', 'Loan Count', 'Total Outstanding', 'Total Principal', 'Total Interest']
    ...
    for bucket in report_data['buckets']:
        ...
        outstanding_cell = ws.cell(row=current_row, column=4, value=float(bucket['total_outstanding']))
        outstanding_cell.number_format = '#,##0.00'
        outstanding_cell.alignment = Alignment(horizontal='right')
        
        principal_cell = ws.cell(row=current_row, column=5, value=float(bucket['total_principal']))
        principal_cell.number_format = '#,##0.00'
        principal_cell.alignment = Alignment(horizontal='right')
        
        interest_cell = ws.cell(row=current_row, column=6, value=float(bucket['total_interest']))
    ...
    current_row += 1
    add_total_row(ws, current_row, 'TOTAL', float(report_data['total_outstanding']), value_col=4)

    if report_data.get('loans_detail'):
        ws_detail = wb.create_sheet("Loan Details")
        
        current_row = add_title_section(ws_detail, "LOAN AGING REPORT - DETAILS")

        headers = ['Loan ID', 'Borrower', 'Principal', 'Outstanding', 'Interest', 'Due Date', 'Days Overdue', 'Status']
        ...
        for loan in report_data['loans_detail']:
            ws_detail.cell(row=current_row, column=1, value=str(loan['loan_id']))
            ws_detail.cell(row=current_row, column=2, value=loan['borrower_name'])
            
            principal_cell = ws_detail.cell(row=current_row, column=3, value=float(loan['principal_amount']))
            principal_cell.number_format = '#,##0.00'
            principal_cell.alignment = Alignment(horizontal='right')
            
            outstanding_cell = ws_detail.cell(row=current_row, column=4, value=float(loan['outstanding_principal']))
            outstanding_cell.number_format = '#,##0.00'
            outstanding_cell.alignment = Alignment(horizontal='right')
            
            interest_cell = ws_detail.cell(row=current_row, column=5, value=float(loan['accrued_interest']))
            interest_cell.number_format = '#,##0.00'
            interest_cell.alignment = Alignment(horizontal='right')
            
            ws_detail.cell(row=current_row, column=6, value=loan['due_date'].strftime('%Y-%m-%d'))
            ws_detail.cell(row=current_row, column=7, value=loan['days_overdue'])
...
def export_account_balance_pdf(account_data: Dict[str, Any]) -> bytes:
    ...
    story.append(Paragraph(f"As of: {account_data['as_of_date'].strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal']))
    ...
    table = Table(table_data, colWidths=[1.5*inch, 2*inch, 1.2*inch, 1.5*inch])
...
def export_transaction_history_pdf(export_data: Dict[str, Any]) -> bytes:
    ...
    table_data = [['Transaction ID', 'Date', 'Description', 'Debit', 'Credit', 'Balance']]
    
    for trans in export_data.get('transactions', [])[:100]:
        table_data.append([
            str(trans.get('transaction_id', '')),
            trans.get('date', '').strftime('%Y-%m-%d') if hasattr(trans.get('date'), 'strftime') else str(trans.get('date', '')),
    ...
    table = Table(table_data, colWidths=[1.2*inch, 1.2*inch, 2.5*inch, 1.2*inch, 1.2*inch, 1.2*inch])
    table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('1F4E78')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
...
def export_trial_balance_pdf(export_data: Dict[str, Any]) -> bytes:
    ...
    story.append(Paragraph(f"As of: {export_data['as_of_date'].strftime('%Y-%m-%d')}", styles['Normal']))
    story.append(Spacer(1, 0.2*inch))
    ...
    balance_status = "✓ BALANCED" if export_data['is_balanced'] else "✗ NOT BALANCED"
...
def export_balance_sheet_pdf(export_data: Dict[str, Any]) -> bytes:
    ...
    assets_data.append(['', 'Total Assets', f"${float(export_data['total_assets']):,.2f}"])
    
    assets_table = Table(assets_data, colWidths=[1.5*inch, 2.5*inch, 1.5*inch])
    ...
    liab_data.append(['', 'Total Liabilities', f"${float(export_data['total_liabilities']):,.2f}"])
    
    liab_table = Table(liab_data, colWidths=[1.5*inch, 2.5*inch, 1.5*inch])
    ...
    eq_data.append(['', 'Total Equity', f"${float(export_data['total_equity']):,.2f}"])
...
def export_loan_aging_pdf(export_data: Dict[str, Any]) -> bytes:
    ...
    summary_data = [['Aging Bucket', 'Days Overdue', 'Count', 'Principal', 'Interest', 'Total']]
    ...
    summary_table = Table(summary_data, colWidths=[1.2*inch, 1.2*inch, 0.8*inch, 1.2*inch, 1.2*inch, 1.2*inch])
    ...
    story.append(Paragraph(f"Total Outstanding: ${float(total_outstanding):,.2f}", styles['Normal']))
