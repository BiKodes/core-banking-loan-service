...
from src.journal.models import JournalEntryLine
...
def account_balance_report(request, account_id):
    ...
    if as_of_date_str:
        try:
            as_of_date = datetime.fromisoformat(
                as_of_date_str.replace('Z', '+00:00')
            )
        except ValueError:
            return Response(
    ...
    if export_format == 'xlsx':
        ...
        return get_excel_response(
            f'account_balance_{account.code}_{as_of_date.date()}', wb
        )
        ...
        return get_pdf_response(
            f'account_balance_{account.code}_{as_of_date.date()}', pdf_bytes
        )
...
def transaction_history_report(request, account_id):
    ...
    if export_format == 'xlsx':
        ...
        return get_excel_response(
            f'transaction_history_{account.code}_{datetime.now().date()}', wb
        )
        ...
        return get_pdf_response(
            f'transaction_history_{account.code}_{datetime.now().date()}',
            pdf_bytes,
        )
...
def loan_aging_report(request):
    ...
    currency = request.query_params.get('currency')
    include_detail = (
        request.query_params.get('include_detail', 'false').lower() == 'true'
    )
    ...
    loans_qs = Loan.objects.filter(
        status__in=['ACTIVE', 'DEFAULTED'], outstanding_principal__gt=0
    ...
    for loan in loans_qs:
        ...
        if bucket_key:
            ...
            total_loans += 1
            total_outstanding += (
                loan.outstanding_principal + loan.accrued_interest
            )
    ...
    for _key, bucket in buckets.items():
        ...
        total_principal = sum(
            loan['outstanding_principal'] for loan in bucket['loans']
        )
        total_interest = sum(
            loan['accrued_interest'] for loan in bucket['loans']
        )
