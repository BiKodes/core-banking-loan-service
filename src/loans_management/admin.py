...
class LoanAdmin(admin.ModelAdmin):
	list_display = ['id', 'borrower', 'lender', 'principal_amount', 'status', 'currency', 'created_at']
	list_filter = ['status', 'currency', 'lender']
	search_fields = ['id', 'borrower__full_name']
	readonly_fields = ['created_at', 'updated_at', 'disbursed_at', 'outstanding_principal', 'accrued_interest', 'version']
	ordering = ['-created_at']


@admin.register(LoanRepayment)
class LoanRepaymentAdmin(admin.ModelAdmin):
	list_display = ['id', 'loan', 'amount', 'principal_component', 'interest_component', 'status', 'paid_at']
