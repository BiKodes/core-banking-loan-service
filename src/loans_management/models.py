...
class Loan(TimeStampedModel):
	...
	origination_fee = models.DecimalField(max_digits=14, decimal_places=2, default=Decimal("0.00"))
	...
	accrued_interest = models.DecimalField(max_digits=14, decimal_places=2, default=Decimal("0.00"))
	...
	def save(self, *args, **kwargs):
		...
		if self.pk:
			...
			if current and current.version != self.version:
				raise ValidationError("Concurrent update detected. Please reload and retry.")
	...
	def update_outstanding(self, principal_delta: Decimal, interest_delta: Decimal = Decimal("0.00")):
		"""Utility to adjust outstanding balances safely."""
		self.outstanding_principal = (self.outstanding_principal or Decimal("0.00")) + principal_delta
		self.accrued_interest = (self.accrued_interest or Decimal("0.00")) + interest_delta
...
class LoanRepayment(TimeStampedModel):
	...
	def clean(self):
		...
		if self.principal_component + self.interest_component != self.amount:
			raise ValidationError("Principal + interest must equal total amount.")
